...
public class ThreeParallelArrays {
	...
	while(true) {
		String state = JOptionPane.showInputDialog("Enter a state name");
		...
		if(index!=-1) {
			String abbreviation = statesAbbreviation[index];
			String capital = capitals[index];
			JOptionPane.showMessageDialog(null, "The abbreviation for " +state+ " is " +abbreviation+ " and the capital is " +capital+ "\n");
			String zipcode = JOptionPane.showInputDialog(null, "Enter the zip code for "+capital);
			Array.set(capitals, index, capital+ " - " +zipcode);	
		}
		else {System.out.print("That is not a correct value, try again.\n");
